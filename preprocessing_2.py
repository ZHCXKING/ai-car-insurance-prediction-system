...
# %%
# 查看数据情况，确认是否有需要的列
filepath = 'data/MergerAWM.xlsx'
columns = ['Age', 'Driving experience', 'Occupation', 'Model',
           'Coverage', 'Insurer', 'Premium', 'Make', 'Year of manufacturer', 'NCD']
np.random.seed(42)  #生成随机日期时用的种子


def infomation():
    df = pd.read_excel(filepath, usecols=columns)
    df.rename(columns={'Driving experience': 'Driving exp', 'Year of manufacturer': 'Year'}, inplace=True)
    start_date = pd.Timestamp('2023-01-01')
    end_date = pd.Timestamp('2023-12-31')
    random_days = np.random.randint(start_date.value, end_date.value, size=len(df), dtype=np.int64)
    df['Settlement date'] = pd.to_datetime(random_days)
    print(df.info())
    print(df.duplicated())
...
def cleanned(df):
    ...
    #处理总额保费
    df['Premium'] = df['Premium'].str.replace('$', '').str.replace(',', '').astype(float)
    #处理保单日期
...
df = df[cols]
df.to_excel('data/2MergerAWM.xlsx', sheet_name='Sheet1', index=False)
# %%
# 使用保存好的模型进行编码
...
df.to_excel('data/3MergerAWM.xlsx', sheet_name='Sheet1', index=False)